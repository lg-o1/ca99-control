...
pitch-direction.js ← 高低音方向感（零基础音高启蒙：弹一个参考音+方向⬆️/⬇️→你在琴上随便弹一个更高/更低的音，方向对且差得够明显就算对，不要求弹准某个音；大跳/五度/三度/一步之遥四关，连击越高门槛越小，纯逻辑出题/方向判定/状态机，18 单元测试）
    ...
    sheet-index.js     ← 📖 课本谱面叠加引擎（当一首 .mid 旁有同名文件夹含 index.json + 每小节照片切片 m001.png…，把真实课本谱面拼成横向卷帘 + 跟随播放高亮当前小节：parseSheetIndex 规范化小节并累加 x0/x1 像素偏移、measureAtBeat 按拍位把整曲均匀映射到 n 个小节[随变速/慢练自动跟随]、cursorX 算光标源像素位、sheetPaths/pngUrl 路径派生；显示高度与切图源高度解耦——defaultSheetHeight 按视口高×比例自适应[横屏收窄/竖屏放大]、clampSheetHeight 夹到 [72,360] 供手动滑杆调节，纯逻辑，13 单元测试）
    app.js            ← 主入口 + 各玩法模块
...
- 📖 课本谱面叠加用 `sheet-index.js`（纯逻辑，13 单元测试）：约定<b>一首 .mid 旁可放一个与文件名同名的文件夹</b>（去掉 `.mid` 后缀），内含 `index.json` + 每小节照片切片 `m001.png m002.png…`（零填充命名、字典序=演奏序，由用户自己的 OMR 切图流程把整页课本照片按小节切出，<b>各切片高度只需一致即可、取任意值</b>——`index.json.height` 只作缩放基准，app 显示高度与之无关）。`index.json` 契约：`{stem, height, bpm, bar_seconds, n_measures, total_seconds, measures:[{i, file, w, t_start, t_end, low_confidence}]}`。构建脚本 `build_midi_catalog.py` 在生成 catalog 时若检测到 `<类别>/<stem>/index.json` 存在就给该曲打 `sheet:true` 标记（`song-tree.js` 按引用透传，无需改），选曲器据此渲染 <b>📖 谱</b> 角标，并决定是否把 `s.path` 作为 sheetPath 传入（非课本曲绝不发起 fetch）。引擎：`parseSheetIndex(json)` 规范化各小节并<b>累加出每格的源像素区间 x0/x1 与 totalWidth</b>（卷帘是把所有小节切片首尾相接铺成一长条）；`measureAtBeat(beat, totalBeats, nMeasures)` 是<b>核心映射</b>——把整曲 `[0,totalBeats)` 均匀分到 n 个课本小节（`span=totalBeats/n`），返回 `{idx, f}`（当前第几格 + 格内进度 0..1）。<b>刻意用「拍位」而非 index.json 的 `t_start/t_end` 秒数驱动光标</b>：拍位来自引擎 `sf.beatAt(t)`，会随 `timeScale`（慢练 60%、🐇 渐进提速）自动跟随，也对 OMR 与 MIDI 小节数不完全一致更稳健（秒数会随变速漂移，只留作 `measureAtTime` 兜底）；`cursorX(measures,idx,f)` 算光标在源像素中的位置。UI（scf 的 `loadSheetFor/buildSheetRibbon/drawSheet/scrollSheetTo`、演奏台对应 `psLoadSheet/psBuildSheet/psDrawSheet/psScrollSheet`）：选曲后从 `getCurrentSong()._sheetPath` 派生路径 fetch index.json + 各 png，在五线谱上方插一条 `.scf-sheet-ribbon`（`overflow-x:auto`）内含 `.scf-sheet-inner`（宽=totalWidth×scale，scale=显示高 ÷ 切图源高，<b>显示高与 index.json 的 height 解耦</b>——切图源高只是缩放基准，可任意；显示高 `sheetActiveH()` 优先读 localStorage 手动值，否则 `defaultSheetHeight(window.innerHeight)` 按视口高×0.24 自适应并夹到 [72,360]，<b>平板横屏视口矮→谱面收窄给键盘/光柱留空间、竖屏视口高→谱面放大更易读</b>；面板头部有<b>高度滑杆</b>（72–360，拖动即 `setSheetH` 存 localStorage 并重排，宽高同比缩放保持纵横比）+ <b>⤢ 自适应</b>按钮（清掉手动值回到跟随视口），偏好<b>两 tab 共享并跨刷新持久</b>；监听 `resize`/横竖屏切换，未手动设过高度时自动重排）平铺 `<img>` 切片 + 一条<b>内容坐标系</b>里绝对定位的光标线（放在 inner 内才能随图片一起滚动）；播放/跟弹的每一帧 `drawSheet(t)` 用 `measureAtBeat` 算出当前格与格内进度 → 移动光标 + 设 `scrollLeft` 居中 + 给当前格加 `.cur` 高亮 + 更新「第 X / N 小节」标签（`low_confidence` 的格用淡黄 `.low-conf` 标注「识别可能不准」），并提供 ◀▶ 手动翻看。起步曲/上传曲无 `_sheetPath` 故自动清空面板，整个特性对非课本曲零副作用
