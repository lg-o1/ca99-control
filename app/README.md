...
- **🐉 Boss 战游戏壳（`boss-battle.js`）**：把"反复练难片段"包装成打怪——`BossBattle` 状态机管理 Boss 血量（50 格）、生命 ❤、乐句位置 `idx`、连击与评星。`press(midi)` 按顺序判定：弹对 → `idx++`、走完整段乐句 → 削血 `dmgPerPass=ceil(maxHp/reps)`（默认 3 遍击败）；弹错 → 扣一颗 ❤、乐句从头来、<b>已削的血不回</b>（低挫败：给容错而非重置）；掉光心 → `failed`（温和提示再来）。`octaveAgnostic`（默认开）让任意八度同名键都算对。`stars()`：无伤且每遍干净=3 星、掉≤1 心=2、否则 1。UI（`renderBossBattle`）：Boss 选择 chips + 血条 + ❤ + 乐句进度 chips（当前音黄色高亮）+ 88 键<b>等待模式高亮该弹的键</b>；命中闪绿、削血时 Boss 抖动 + 飘 `-伤害` + 击败撒花 + ⭐；CA99 物理输入经新增 `bossOnNote`/`bossOnNoteOff` 钩子直接判定（与屏幕点击同一条 `handlePress`）。10 条单元测试，成绩入仪表盘。默认归为 🌱 初级（侧栏 `data-level="1"`）。
