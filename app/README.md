...
pitch-direction.js ← 高低音方向感（零基础音高启蒙：弹一个参考音+方向⬆️/⬇️→你在琴上随便弹一个更高/更低的音，方向对且差得够明显就算对，不要求弹准某个音；大跳/五度/三度/一步之遥四关，连击越高门槛越小，纯逻辑出题/方向判定/状态机，18 单元测试）
    ...
    sheet-index.js     ← 📖 课本谱面叠加引擎（当一首 .mid 旁有同名文件夹含 index.json + 每小节照片切片 m001.png…，把真实课本谱面拼成横向卷帘 + 跟随播放高亮当前小节：parseSheetIndex 规范化小节并累加 x0/x1 像素偏移、measureAtBeat 按拍位把整曲均匀映射到 n 个小节[随变速/慢练自动跟随]、cursorX 算光标源像素位、sheetPaths/pngUrl 路径派生，纯逻辑，13 单元测试）
    app.js            ← 主入口 + 各玩法模块
