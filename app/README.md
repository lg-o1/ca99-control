...
- **🚀 极速挑战游戏壳（`speed-run.js`）**：把"练快"从"跟节拍器卡点"（最劝退易放弃孩子）改成"<b>刷自己的纪录</b>"——`SpeedRun` 状态机按顺序判定整段乐句，`press(midi, nowMs)` 在第一个音记 `runStartMs`、最后一个音用 `bpmFromElapsed(noteCount, elapsed, beatsPerNote)` 把耗时换算成<b>有效 BPM</b>；`effBpm >= targetBpm` → 升档 `targetBpm += step`(默认 +5)；`effBpm > bestBpm` → 刷新个人纪录；弹错 → 整段作废 `idx=0` 从头来（<b>不惩罚 targetBpm</b>，低挫败）。`octaveAgnostic`（默认开）。`history[]` 记录每次完成的 BPM 供画成长曲线。UI（`renderSpeedRun`）：乐句选择 chips + 三仪表盘（🎯 目标/🏅 纪录/⏱️ 上次）+ <b>速度成长曲线</b> sparkline（金点=刷纪录）+ 88 键<b>等待模式高亮该弹的键</b>；命中闪绿、错音闪红 + "从头再来～别急"、升档时 `cheerToast`+`cheerBurst` 庆祝 + `recordPractice`。CA99 物理输入经新增 `speedRunOnNote`/`speedRunOnNoteOff` 钩子（与屏幕点击同一条 `handlePress`，注入 `performance.now()` 计时）。8 条单元测试，成绩入仪表盘。默认归为 🌱 初级（侧栏 `data-level="1"`）。
