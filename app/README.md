...
- **留存与挫败感优化层（复用 practice-stats，零新模块）**：针对易放弃的初学者补了一层<b>「鼓励机器」外壳</b>，全部复用已有的 `practice-stats.js` 引擎 + `cheerBurst`/`cheerToast`，不新增模块/依赖。① <b>成长型措辞</b>——把全 app 23 处练习反馈里生硬的 `❌` 全改成温和「再试试 / 差一点～ / 还差一点」加 🤔 表情（仅 5 处技术性「解析/识别失败」提示保留 ❌），直接对抗固定思维。② <b>错误配色软化</b>——`.sight-feedback.no` 由刺眼红 `#ff6b81` 改为琥珀 `#fbbf24`，让「答错」读起来像「快了/还没」而非惩罚（一处 CSS 全模块生效）。③ <b>顶栏每日鼓励横幅</b> `#daily-strip`（顶栏下方细条，`renderDailyStrip()`）——用 `practiceStats.snapshot().dayStreak` + `recentDays(1)[0].sessions` 显示「🔥 连练 N 天 · 🎯 今日目标 ▓▓░ X/3」（微目标默认 3 次、超小化降抗拒），在 `main()` 与每次 `recordPractice()` 后刷新，达成变暖色「🎉 今日目标达成」。④ <b>解锁成就主动庆祝</b>——`recordPractice` 里新解锁的成就不再只写日志，`achievementCelebrate(a)` 弹 `cheerToast(🏅 解锁成就…)` + 全屏 `position:fixed` 撒花层（多个成就 1.4s 错峰）。<b>坑点</b>：`practiceStats`/`recordPractice` 同为模块作用域不挂 `window`，冒烟需走真实模块交互（开始视奏闪卡→点琴键作答→停止触发 record）来验证横幅自增与成就弹窗；首个成就「第一步」若 localStorage 已解锁则不会重弹，验证前需 `localStorage.removeItem('ca99-practice-stats')` 再刷新。
