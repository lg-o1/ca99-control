...
- 音阶指法提示用 `scale-fingering.js`（纯逻辑，113 单元测试）：内置 9 个调的标准指法表 `FINGERINGS`（C/G/D/A/E/F 大调 + A/E/D 自然小调，每条含一个八度的半音偏移 `offsets`、右手 `rh`、左手 `lh` 指法，长度均为 8）——C/G/D/A/E 右手 1 2 3 1 2 3 4 5、F 右手例外 1 2 3 4 1 2 3 4、左手统一 5 4 3 2 1 3 2 1。`scaleNotes(id,rootMidi)` 生成 MIDI 序列、`crossingPoints(fingerArr,hand)` 算穿指/跨指点（右手上行=拇指处 finger===1、左手上行=前一指为 1 而当前非 1）。`ScaleFingeringSession(id,{hand,bidirectional,tolerateOctave})` 是跟弹会话：`feed(note)` 弹对当前音才前进（默认八度等价容忍）、弹错记一次不前进、给出 `currentNote()`/`currentFinger()`，`onAdvance`/`onComplete` 回调，`accuracy`/`summary()`。UI 把音阶画成"谱面"——每格显示手指号+音名，当前音绿框高亮、已弹半透明、穿指点橙色标 ↩，可🔊听一遍演示，跟弹完成记进仪表盘。和"音阶练习""音阶八度跨度"都不同——这里专练<b>正确指法与穿指动作</b> |
...
| 🏆 成就仪表盘 | ✅ | 汇总视奏/听辨/力度/音阶/节奏/旋律/和弦进行/节拍稳定度/双手协调/琶音跑动/连奏断奏/踏板时机/颤音速度/装饰音/音程大跳/旋律突出/力度渐变/速度渐变/复节奏/颗粒性/手指独立性/音阶八度跨度/节奏听写/移调视奏/和弦转位听辨/调号识别/音阶指法提示各训练成绩，统计总练习次数/正确率/连续天数/最佳连击，最近 7 天柱状图、模块细分表、10 枚成就徽章墙（已解锁/即将解锁/锁定） || 📡 MIDI 监视器 | ✅ | 实时显示钢琴发来的音符/CC/SysEx |
