...
> 侧边栏按「演奏控制 / 练习训练 / 工具」三组分类，每组标题带模块数量徽章，<b>顶部带难度过滤条</b>（🌱 初级 / 🌿 进阶 / 🔥 全部，默认<b>初级</b>，记忆到 localStorage）——模块多达 80 个，默认只列出 33 个适合零基础的「初级」玩法，避免一上来就被复杂功能淹没；切「进阶」加显中级（共 59 个）、「全部」显示所有。每个 `.nav-btn` 用 `data-level="1|2|3"` 标级，`setupNavSearch()` 里 `levelOk()` 按「≤ 当前级别」过滤（初级=1、进阶=1+2、全部=不限）。其下是 🔍 实时搜索框（输入即过滤模块，Esc 清空）——<b>搜索时跨全部级别</b>（显式搜索意图优先，即便处于初级也能搜到高级模块），清空后恢复按级别过滤。可滚动；切换模块时顶栏面包屑显示当前模块名、内容区自动回到顶部；窄屏（≤1024px 收窄、≤720px 折叠为图标条）自适应。各模块标题带渐变色条、统计数字用渐变描字，整体配色/动画统一。
