...
function switchModule(name) {
  ...
  try { document.dispatchEvent(new CustomEvent('ca99:module-change', { detail: name })); } catch (_) {}
}
...
function renderScoreFollow() {
  ...
  function scfPositionSheet(wrap, inner, cur, x, idx) {
    ...
    if (scfSheetMode === 'half') {
      ...
      const lOff = Math.max(0, halfW - (endOf(leftIdx) - hps[leftIdx].startX));
      const ls = $('#scf-half-l-strip'), rs = $('#scf-half-r-strip'), hc = $('#scf-half-cursor');
      if (ls) { ls.style.width = endOf(leftIdx) + 'px'; ls.style.overflow = 'hidden'; ls.style.transform = `translateX(${(lOff - hps[leftIdx].startX).toFixed(1)}px)`; }
      if (rs) { rs.style.width = endOf(rightIdx) + 'px'; rs.style.overflow = 'hidden'; rs.style.transform = `translateX(${(-hps[rightIdx].startX).toFixed(1)}px)`; }
...
// ---------- 🪄 魔法五声即兴沙盒（Magic Jam，零失败自由创作） ----------
// 把键盘「锁」到五声音阶：怎么弹都好听。无评分、无对错、无 game-over——
// 给「爱音乐讨厌练琴」的孩子一个重新「玩音乐」的低能量出口。
// 真琴弹任何键 → 就近吸附到五声音 → 点亮该键 + 火花（钢琴自身发声）；
// 屏幕点琴键/音垫 → 吸附后用 Web Audio 发声。可叠加 🥁 律动伴奏（仅浏览器，绝不发往 CA99）。
function renderMagicJam() {
  const root = $('#module-magicjam');
  if (!root) return;
  const MJ_ROOT = 'ca99_mj_root', MJ_MOOD = 'ca99_mj_mood', MJ_BPM = 'ca99_mj_bpm';
  let rootPc = 0, moodId = 'major', bpm = 92, soundOn = true;
  try {
    const r = parseInt(localStorage.getItem(MJ_ROOT), 10); if (MagicJam.ROOTS.some(x => x.pc === r)) rootPc = r;
    const md = localStorage.getItem(MJ_MOOD); if (MagicJam.MOODS.some(x => x.id === md)) moodId = md;
    const bp = parseInt(localStorage.getItem(MJ_BPM), 10); if (bp >= 60 && bp <= 140) bpm = bp;
  } catch (_) {}
  const moodType = () => (MagicJam.MOODS.find(m => m.id === moodId) || MagicJam.MOODS[0]).type;

  root.innerHTML = `
    <h2 style="margin-bottom:6px">🪄 魔法即兴沙盒</h2>
    <p style="color:var(--muted);margin-bottom:14px">键盘被施了<b>魔法</b>——只会响出<b>好听</b>的音！随便弹、怎么弹都不会错 🎵。下面<b>彩色的键</b>就是魔法键，弹它们最稳；接上 CA99 弹<b>任何</b>键都会自动「吸」到最近的好听音上。想更带感就打开 🥁 <b>律动伴奏</b>，跟着鼓点一起即兴。<b>没有分数、没有对错</b>，只管玩。</p>

    <div class="card-panel">
      <div class="param-row"><label>选个调</label>
        <div id="mj-roots" class="ear-chips">${MagicJam.ROOTS.map(r => `<button class="ear-chip${r.pc === rootPc ? ' on' : ''}" data-pc="${r.pc}">${r.name}</button>`).join('')}</div>
      </div>
      <div class="param-row"><label>心情</label>
        <div id="mj-moods" class="ear-chips">${MagicJam.MOODS.map(m => `<button class="ear-chip${m.id === moodId ? ' on' : ''}" data-m="${m.id}">${m.label}</button>`).join('')}</div>
      </div>
      <div class="param-row" style="gap:18px;flex-wrap:wrap;align-items:center">
        <button id="mj-groove" class="mini-btn">🥁 律动伴奏：关</button>
        <label style="display:flex;align-items:center;gap:8px">速度 <input type="range" id="mj-bpm" min="60" max="140" step="2" value="${bpm}"> <b id="mj-bpmv">${bpm}</b></label>
        <label class="ls-check"><input type="checkbox" id="mj-sound" checked> 🔊 点键发声</label>
      </div>
    </div>

    <div id="mj-stage" class="ls-stage">
      <div id="mj-fxlayer" class="ls-fxlayer"></div>
      <div class="ls-hud">
        <div class="ls-combo" id="mj-combo"></div>
        <div class="ls-stat">🎵 弹了 <b id="mj-total">0</b> 个音 · 🔥 最长连弹 <b id="mj-max">0</b></div>
      </div>
      <div class="ls-hint" id="mj-hint">点彩色音垫或下面的琴键，开始你的即兴 ✨</div>
    </div>

    <div class="mj-pads" id="mj-pads"></div>

    <div class="kb-wrap">
      <div class="kb-cap">🎹 <b>彩色键</b>＝魔法五声键。接 CA99 弹任何键都会自动吸到最近的好听音</div>
      <div id="mj-kb"></div>
    </div>`;

  const stage = $('#mj-stage');
  const fxLayer = $('#mj-fxlayer');
  const hintEl = $('#mj-hint');
  const comboEl = $('#mj-combo');
  const totalEl = $('#mj-total');
  const maxEl = $('#mj-max');
  const padsEl = $('#mj-pads');
  let hintHidden = false;
  let total = 0, combo = 0, maxCombo = 0, lastHit = 0, recordedBlocks = 0;

  const kb = new PianoKeyboard($('#mj-kb'), { first: 21, last: 108, labels: 'c' });

  // 当前五声音的「音级 → 彩虹序号」映射，给魔法键/音垫统一上色
  ...
  // 把五声「魔法键」高亮到 88 键上 + 重建彩色音垫
  function applyScale() {
    const type = moodType();
    const scaleMidis = MagicJam.scaleMidis(rootPc, type, 36, 96);
    kb.highlightMany(scaleMidis.map(m => ({ midi: m, color: pcDegreeColor(m) })));
    // 音垫：从 C4 区起的 10 个连续五声音，触屏友好
    const pads = MagicJam.padNotes(rootPc, type, 10, 60);
    padsEl.innerHTML = pads.map((m, i) => {
      const c = MagicJam.padColor(i);
      const name = (typeof CA99 !== 'undefined' && CA99.noteName) ? CA99.noteName(m) : String(m);
      return `<button class="mj-pad" data-m="${m}" style="--pc:${c}">${name}</button>`;
    }).join('');
    padsEl.querySelectorAll('.mj-pad').forEach(b => {
      b.onclick = () => play(parseInt(b.dataset.m, 10), 100, false);
    });
  }

  function keyFrac(midi) { return Math.max(0, Math.min(1, (midi - 21) / (108 - 21))); }

  // 在舞台对应位置喷一束小火花（数量随力度），纯视觉
  function spark(midi, vel, color) {
    const W = stage.clientWidth || 600;
    const x = keyFrac(midi) * W;
    const baseY = stage.clientHeight || 220;
    const count = 6 + Math.round((vel / 127) * 10);
    const topY = baseY - 30 - (vel / 127) * (baseY * 0.4);
    const beam = document.createElement('div');
    beam.className = 'ls-beam';
    beam.style.cssText = `left:${x}px;--bh:${(40 + (vel / 127) * (baseY - 60)).toFixed(0)}px;--bc:${color}`;
    fxLayer.appendChild(beam);
    setTimeout(() => beam.remove(), 720);
    for (let k = 0; k < count; k++) {
      const p = document.createElement('i');
      const ang = (Math.PI * 2 * k) / count + Math.random() * 0.6;
      const dist = (24 + (vel / 127) * 34) * (0.5 + Math.random() * 0.8);
      const dx = Math.cos(ang) * dist, dy = Math.sin(ang) * dist - 16;
      const dur = 0.5 + Math.random() * 0.4;
  ...
  // 核心：弹一个音。fromScreen=true 时用 Web Audio 发声；真琴由钢琴自身发声。
  function play(rawMidi, vel, fromScreen) {
    const snapped = MagicJam.snapToScale(rawMidi, rootPc, moodType());
    if (!hintHidden) { hintEl.style.display = 'none'; hintHidden = true; }
    const color = pcDegreeColor(snapped);
    kb.press(snapped);
    kb.flash(snapped, color);
    setTimeout(() => kb.release(snapped), 260);
    if (fromScreen && soundOn) { try { playTone(midiToFreq(snapped), 0, 0.6, 0.2); } catch (_) {} }
    spark(snapped, vel, color);
    // 连弹计数（1.4s 内连续即累加）——纯鼓励，不评分
    const now = performance.now();
    ...
    maxEl.textContent = maxCombo;
    if (combo >= 2) comboEl.textContent = `× ${combo}`;
    if (combo === 8 || combo === 16 || combo === 30) {
      comboEl.textContent = `🔥 连弹 ${combo}！`;
      comboEl.classList.remove('pop'); void comboEl.offsetWidth; comboEl.classList.add('pop');
      try { cheerBurst(stage, 40); } catch (_) {}
    }
    // 每弹 24 个音记一次练习，喂连胜/今日目标/仪表盘（自由玩耍也算练琴）
    const block = Math.floor(total / 24);
    if (block > recordedBlocks) {
      recordedBlocks = block;
      recordPractice('magicjam', '魔法即兴', 1, 1, maxCombo);
    }
  }

  // 屏幕点 88 键：吸附后发声 + 点亮
  kb.onNoteOn = (m) => play(m, 100, true);
  kb.onNoteOff = (m) => kb.release(MagicJam.snapToScale(m, rootPc, moodType()));

  // ---- 🥁 律动伴奏（仅浏览器 Web Audio，绝不发往 CA99）----
  let ac = null, grooveOn = false, grooveTimer = null, noiseBuf = null;
  function ensureAC() {
  ...
  function drum(ctx, kind, when) {
    if (kind === 'kick') {
      const o = ctx.createOscillator(), g = ctx.createGain();
      o.type = 'sine'; o.frequency.setValueAtTime(130, when); o.frequency.exponentialRampToValueAtTime(45, when + 0.12);
      g.gain.setValueAtTime(0.0001, when); g.gain.exponentialRampToValueAtTime(0.9, when + 0.005); g.gain.exponentialRampToValueAtTime(0.0001, when + 0.16);
      o.connect(g).connect(ctx.destination); o.start(when); o.stop(when + 0.2);
    } else {
      const src = ctx.createBufferSource(); src.buffer = getNoise(ctx);
      const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = kind === 'hat' ? 7000 : 1600;
      const g = ctx.createGain();
      const peak = kind === 'hat' ? 0.18 : 0.4, dec = kind === 'hat' ? 0.04 : 0.14;
      g.gain.setValueAtTime(peak, when); g.gain.exponentialRampToValueAtTime(0.0001, when + dec);
      src.connect(hp).connect(g).connect(ctx.destination); src.start(when); src.stop(when + dec + 0.02);
    }
  }
  function bassNote(ctx, midi, when, dur) {
    const o = ctx.createOscillator(), g = ctx.createGain();
    o.type = 'triangle'; o.frequency.value = midiToFreq(midi);
    g.gain.setValueAtTime(0.0001, when); g.gain.exponentialRampToValueAtTime(0.5, when + 0.02); g.gain.exponentialRampToValueAtTime(0.0001, when + dur);
    o.connect(g).connect(ctx.destination); o.start(when); o.stop(when + dur + 0.05);
  }
  function scheduleBar(ctx) {
    if (!grooveOn) return;
    const spb = 60 / bpm;
    const t0 = ctx.currentTime + 0.06;
    MagicJam.vampBar(rootPc, 1).forEach(e => {
      const when = t0 + e.beat * spb;
      if (e.kind === 'bass') bassNote(ctx, e.midi, when, spb * 0.9);
      else drum(ctx, e.kind, when);
    });
    grooveTimer = setTimeout(() => scheduleBar(ctx), spb * 4 * 1000);
  }
  function setGroove(on) {
    grooveOn = on;
    const btn = $('#mj-groove');
    if (on) {
      const ctx = ensureAC(); if (!ctx) { grooveOn = false; return; }
      btn.textContent = '🥁 律动伴奏：开'; btn.classList.add('on');
      scheduleBar(ctx);
    } else {
      btn.textContent = '🥁 律动伴奏：关'; btn.classList.remove('on');
      if (grooveTimer) clearTimeout(grooveTimer); grooveTimer = null;
    }
  }

  // ---- 控件接线 ----
  $('#mj-roots').querySelectorAll('button').forEach(b => b.onclick = () => {
    rootPc = parseInt(b.dataset.pc, 10);
    $('#mj-roots').querySelectorAll('button').forEach(x => x.classList.toggle('on', x === b));
    try { localStorage.setItem(MJ_ROOT, String(rootPc)); } catch (_) {}
    applyScale();
  });
  $('#mj-moods').querySelectorAll('button').forEach(b => b.onclick = () => {
    moodId = b.dataset.m;
    $('#mj-moods').querySelectorAll('button').forEach(x => x.classList.toggle('on', x === b));
    try { localStorage.setItem(MJ_MOOD, moodId); } catch (_) {}
    applyScale();
  });
  $('#mj-groove').onclick = () => setGroove(!grooveOn);
  $('#mj-bpm').oninput = (e) => {
    bpm = parseInt(e.target.value, 10);
    $('#mj-bpmv').textContent = bpm;
    try { localStorage.setItem(MJ_BPM, String(bpm)); } catch (_) {}
  };
  $('#mj-sound').onchange = (e) => { soundOn = e.target.checked; };

  // 切走模块时自动停伴奏（避免后台一直响）
  document.addEventListener('ca99:module-change', (ev) => {
    if (ev.detail !== 'magicjam' && grooveOn) setGroove(false);
  });

  // 真琴输入：吸附到五声音 → 点亮 + 火花（钢琴自身发声，不在此合成）
  magicJamOnNote = (midi, vel) => play(midi, vel == null ? 90 : vel, false);
  magicJamOffNote = (midi) => kb.release(MagicJam.snapToScale(midi, rootPc, moodType()));
...
function renderGuessSong() {
  ...
  root.innerHTML = `
    <h2 style="margin-bottom:6px">🕵️ 猜歌视奏</h2>
    <p style="color:var(--muted);margin-bottom:14px">侦探游戏！屏幕只给<b>五线谱</b>（藏住曲名），你<b>照着谱把开头弹出来</b>——弹完会<b>回放一遍</b>让你听到，然后从下面 4 个选项里<b>猜这是哪首歌</b>，猜对揭晓 🎉！全是大家熟悉的童谣，把"读谱"变成"破案"。可接 CA99 真琴或点屏幕键盘。</p>

    <div class="gs-stats">
      <span class="dw-stat">⭐ 得分 <b id="gs-score">0</b></span>
      <span class="dw-stat">🔥 连对 <b id="gs-streak">0</b></span>
    </div>

    <div class="gs-stage">
      <div class="gs-mystery" id="gs-mystery">🕵️ ？？？</div>
      <div class="sight-staff-wrap"><div id="gs-staff"></div></div>
    </div>
    <div class="bb-feedback" id="gs-feedback">点"新一题"开始破案</div>

    <div class="gs-options" id="gs-options"></div>

    <div class="kb-wrap">
      <div class="kb-cap" id="gs-kbcap">🎹 照着五线谱、按高亮键把旋律弹出来</div>
      <div id="gs-kb"></div>
    </div>

    <div class="rotate-bar">
      <button id="gs-new" class="big-btn">🔍 新一题</button>
      <button id="gs-replay" class="mini-btn" style="display:none">🔊 再听一遍</button>
      <span id="gs-status" style="color:var(--muted)">未开始</span>
    </div>`;

  const CLEF = 'treble';
  function drawStaff() {
    const notes = gs && gs.song ? gs.song.notes : [];
    const W = Math.max(260, 70 + notes.length * 34), H = 150;
    const topY = 48, stepPx = 6;
    const yForPos = (p) => topY + (8 - p) * stepPx;
    let svg = `<svg viewBox="0 0 ${W} ${H}" class="sight-svg" preserveAspectRatio="xMidYMid meet">`;
    for (let p = 0; p <= 8; p += 2) svg += `<line x1="28" y1="${yForPos(p)}" x2="${W - 14}" y2="${yForPos(p)}" class="staff-line"/>`;
    svg += `<text x="34" y="${yForPos(2) + 6}" class="clef-glyph">𝄞</text>`;
    notes.forEach((midi, i) => {
      const pos = staffPosition(midi, CLEF);
      const cx = 78 + i * 34, cy = yForPos(pos);
      if (pos > 8) for (let p = 10; p <= pos; p += 2) svg += `<line x1="${cx - 13}" y1="${yForPos(p)}" x2="${cx + 13}" y2="${yForPos(p)}" class="ledger-line"/>`;
      if (pos < 0) for (let p = -2; p >= pos; p -= 2) svg += `<line x1="${cx - 13}" y1="${yForPos(p)}" x2="${cx + 13}" y2="${yForPos(p)}" class="ledger-line"/>`;
      const cur = gs && gs.phase === 'play' && i === gs.idx;
      const done = gs && (gs.phase !== 'play' || i < gs.idx);
      const fill = cur ? '#fbbf24' : (done ? '#34d399' : '#e6e8f2');
      svg += `<ellipse cx="${cx}" cy="${cy}" rx="6.5" ry="5" transform="rotate(-20 ${cx} ${cy})" style="fill:${fill}"/>`;
      svg += `<line x1="${cx + 6}" y1="${cy}" x2="${cx + 6}" y2="${cy - 30}" class="note-stem"/>`;
    });
    svg += '</svg>';
    $('#gs-staff').innerHTML = svg;
  }

  function showTarget() {
    const t = gs ? gs.current() : null;
    gsKb.clear();
    if (t != null) gsKb.highlightMany([{ midi: t, color: '#fbbf24', text: kbNoteName(t) }]);
  }

  function playbackMelody() {
    if (!gs || !gs.song) return;
    const notes = gs.song.notes;
    notes.forEach((m, i) => setTimeout(() => { playTone(midiToFreq(m), 0, 0.45); gsKb.flash(m, '#a78bfa'); }, i * 380));
  }

  function drawOptions() {
    const wrap = $('#gs-options');
    if (!gs || gs.phase !== 'guess') { wrap.innerHTML = ''; return; }
    wrap.innerHTML = gs.options.map((s) =>
      `<button class="gs-opt" data-id="${s.id}">${s.emoji} ${s.title}</button>`).join('');
    wrap.querySelectorAll('.gs-opt').forEach((el) => {
      el.onclick = () => onGuess(el.dataset.id, el);
    });
  }

  function onGuess(id, el) {
    if (!gs || gs.phase !== 'guess') return;
    const r = gs.guess(id);
    if (r.correct) {
      $('#gs-score').textContent = r.score;
      $('#gs-streak').textContent = r.streak;
      $('#gs-mystery').textContent = `${r.emoji} ${r.title}`;
      $('#gs-mystery').classList.add('revealed');
      el.classList.add('right');
      cheerToast(`🎉 破案啦！是《${r.title}》`, root);
      cheerBurst(root, 70);
      $('#gs-feedback').innerHTML = `🎉 <b>答对了！</b>这是《${r.title}》${r.emoji}——你的耳朵和眼睛都很棒！点"新一题"再来。`;
      $('#gs-status').textContent = '破案成功！';
      recordPractice('guess', '猜歌视奏', gs.rounds, gs.score, gs.streak);
      guessOnNote = null; guessOnNoteOff = null;
      $('#gs-replay').style.display = 'none';
    } else {
      el.classList.add('wrong');
      el.disabled = true;
      $('#gs-feedback').textContent = '🤔 不是这首哦～再想想，听听旋律再猜（不扣分）';
    }
  }

  function handlePress(midi) {
    if (!gs || gs.phase !== 'play') return;
    const r = gs.press(midi);
    if (r.wrong) {
      gsKb.flash(midi, '#f87171');
      $('#gs-feedback').textContent = '🤔 这个音不对～照着五线谱从头再弹（别急）';
      drawStaff(); showTarget();
      return;
    }
    gsKb.flash(midi, '#34d399');
    if (r.playDone) {
      drawStaff();
      gsKb.clear();
      $('#gs-feedback').innerHTML = '👂 听一听你弹出的旋律……<b>这是哪首歌？</b>从下面选！';
      $('#gs-status').textContent = '猜一猜！';
      $('#gs-replay').style.display = '';
      drawOptions();
      setTimeout(playbackMelody, 250);
      guessOnNote = null;
      return;
    }
    drawStaff(); showTarget();
  ...
  $('#gs-new').onclick = () => {
    if (!gs) gs = new GuessSong({ octaveAgnostic: true });
    gs.next();
    $('#gs-mystery').textContent = '🕵️ ？？？';
    $('#gs-mystery').classList.remove('revealed');
    drawStaff();
    drawOptions();
    showTarget();
    $('#gs-feedback').innerHTML = `🔍 照着五线谱<b>把开头弹出来</b>（${gs.song.notes.length} 个音）——按高亮键`;
    $('#gs-status').textContent = '照谱弹奏中…';
    $('#gs-replay').style.display = 'none';
    guessOnNote = (m) => handlePress(m);
    guessOnNoteOff = (m) => { try { gsKb.release(m); } catch (_) {} };
  };
  $('#gs-replay').onclick = playbackMelody;
...
function renderPaddleTones() {
  ...
  if (!root) return;
  let poolId = 'penta';
  let byEar = false;       // 听音接：球空白、落下时发声
  let running = false;
  let game = null, rafId = 0, t0 = 0;
  const floats = [];       // {x,y,life,txt,color}
  const voiced = new Set();
  let lineFlash = 0, lineFlashCol = '#34d399';
  const BEST = 'ca99_paddle_best'; // {poolId: bestCombo}
  ...
  root.innerHTML = `
    <h2 style="margin-bottom:6px">🏓 弹球接音</h2>
    <p style="color:var(--muted);margin-bottom:12px">一颗颗<b>音球</b>从上面掉下来，每颗球带一个音（写着音名）。趁它落到底之前，在钢琴上<b>弹出对应的音</b>，就能把球"弹回去"接住 🏓！接住攒连击，弹错<b>不扣分</b>，漏接也<b>不会输</b>。开<b>🔊 听音接</b>球会变空白、落下时发声，靠<b>耳朵</b>找音（视唱练耳）。可接 CA99 真琴或点屏幕键盘。</p>

    <div class="bb-pick" id="pdl-pick"></div>

    <div class="rotate-bar" style="margin:6px 0 10px">
      <label class="scaffold-toggle">球数
        <select id="pdl-count"><option value="8">8</option><option value="12" selected>12</option><option value="16">16</option></select>
      </label>
      <label class="scaffold-toggle">速度
        <select id="pdl-speed">
          <option value="3400">🐢 慢</option>
          <option value="2600" selected>🎵 中</option>
          <option value="1900">🏃 快</option>
        </select>
      </label>
      <label class="scaffold-toggle"><input type="checkbox" id="pdl-ear"> 🔊 听音接（球空白·靠耳朵）</label>
    </div>

    <div class="dr-hud">
      <span class="dr-stat">🏓 接住 <b id="pdl-caught">0</b></span>
      <span class="dr-stat">💨 漏接 <b id="pdl-miss">0</b></span>
      <span class="dr-stat">🔥 连击 <b id="pdl-combo">0</b></span>
      <span class="dr-stat">🏆 最佳连击 <b id="pdl-best">0</b></span>
    </div>

    <div class="rj-track-wrap">
      <canvas id="pdl-canvas" class="rj-canvas" width="660" height="300"></canvas>
    </div>

    <div class="bb-feedback" id="pdl-feedback">选好音池，按"开始"——音球要掉下来啦！</div>

    <div class="rotate-bar">
      <button id="pdl-start" class="big-btn">▶ 开始</button>
    </div>
  ...
  const cx = cv.getContext('2d');
  const W = cv.width, H = cv.height;
  const TOP = 18, FLOOR = H - 64;            // 地面线（y=1 对应这里）
  const yPix = (y) => TOP + y * (FLOOR - TOP);
  const PADDLE_Y = TOP + 0.45 * (FLOOR - TOP); // 可接区上界视觉参考

  function drawPicker() {
    $('#pdl-pick').innerHTML = PT_POOLS.map((p) =>
      `<button class="bb-chip${p.id === poolId ? ' on' : ''}" data-id="${p.id}">${p.emoji} ${p.name}<small>${p.pcs.length} 个音</small></button>`).join('');
    $('#pdl-pick').querySelectorAll('.bb-chip').forEach((el) => {
      el.onclick = () => { if (running) return; poolId = el.dataset.id; drawPicker(); updateHud(); draw(); };
    });
  }

  function updateHud() {
    $('#pdl-caught').textContent = game ? game.caughtCount : 0;
  ...
  function draw() {
    cx.clearRect(0, 0, W, H);
    const g = cx.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, 'rgba(56,189,248,.10)'); g.addColorStop(1, 'rgba(244,114,182,.08)');
    cx.fillStyle = g; cx.fillRect(0, 0, W, H);
    // 球拍区参考线
    cx.strokeStyle = 'rgba(255,255,255,.18)'; cx.setLineDash([6, 6]); cx.lineWidth = 1.5;
    cx.beginPath(); cx.moveTo(0, PADDLE_Y); cx.lineTo(W, PADDLE_Y); cx.stroke(); cx.setLineDash([]);
    cx.fillStyle = 'rgba(255,255,255,.4)'; cx.font = '11px system-ui,sans-serif';
    cx.textAlign = 'left'; cx.textBaseline = 'middle'; cx.fillText('🏓 在这条线以下接球', 6, PADDLE_Y - 10);
    // 地面线
    cx.save();
    if (lineFlash > 0) { cx.shadowColor = lineFlashCol; cx.shadowBlur = 22 * lineFlash; }
    cx.strokeStyle = lineFlash > 0 ? lineFlashCol : 'rgba(255,255,255,.5)';
    cx.lineWidth = 4; cx.beginPath(); cx.moveTo(0, FLOOR); cx.lineTo(W, FLOOR); cx.stroke();
    cx.restore();
    if (lineFlash > 0) lineFlash = Math.max(0, lineFlash - 0.08);

    if (game) {
      const now = performance.now() - t0;
      const balls = game.activeBalls(now);
      const n = balls.length;
      balls.forEach((b) => {
        const x = W / 2 + Math.sin((b.i + 1) * 1.7) * (W * 0.30); // 分散到不同 x 列
        const y = yPix(Math.min(1.18, b.y));
        const col = BALL_COLORS[b.pc % BALL_COLORS.length];
        const catchable = b.y >= 0.45 && b.y <= 1.12;
        cx.save();
        cx.shadowColor = col; cx.shadowBlur = catchable ? 18 : 8;
        cx.fillStyle = col; cx.globalAlpha = 0.95;
        cx.beginPath(); cx.arc(x, y, 22, 0, Math.PI * 2); cx.fill();
        cx.restore();
        cx.globalAlpha = 1;
        if (!byEar) {
          cx.fillStyle = '#0b1020'; cx.font = 'bold 16px system-ui,sans-serif';
          cx.textAlign = 'center'; cx.textBaseline = 'middle';
          cx.fillText(b.name, x, y + 1);
        } else {
          cx.fillStyle = 'rgba(11,16,32,.6)'; cx.font = 'bold 18px system-ui,sans-serif';
          cx.textAlign = 'center'; cx.textBaseline = 'middle';
          cx.fillText('?', x, y + 1);
        }
      });
      ...
      cx.fillStyle = 'rgba(220,228,255,.55)'; cx.font = '16px system-ui,sans-serif';
      cx.textAlign = 'center'; cx.textBaseline = 'middle';
      cx.fillText('按「开始」让音球掉下来 🏓', W / 2, (TOP + FLOOR) / 2);
    }
    // 漂浮判定字
    for (const f of floats) {
      cx.globalAlpha = Math.max(0, f.life);
  ...
  function ballX(i) { return W / 2 + Math.sin((i + 1) * 1.7) * (W * 0.30); }
  ...
  function loop() {
    ...
    if (byEar) {
      for (const b of game.balls) {
        if (!voiced.has(b.i) && now >= b.spawnMs && !b.caught && !b.missed) {
          voiced.add(b.i);
          try { playTone(midiToFreq(60 + b.pc), 0, 0.7); } catch (_) {}
        }
      }
    }
    const missed = game.expire(now);
    if (missed.length) { lineFlash = 0.6; lineFlashCol = '#fbbf24'; updateHud(); }
    draw();
    if (game.isDone() || now > game.durationMs() + 200) { finish(); return; }
    rafId = requestAnimationFrame(loop);
  }

  function finish() {
    cancelAnimationFrame(rafId);
    running = false; paddleOnNote = null;
    $('#pdl-start').textContent = '▶ 开始'; $('#pdl-start').classList.remove('running');
    const stars = game.stars();
    const starStr = '★'.repeat(stars) + '☆'.repeat(3 - stars);
    recordPractice('paddle', '弹球接音', game.total, game.caughtCount, game.bestCombo);
    const rate = game.catchRate();
    if (stars >= 3) {
      $('#pdl-feedback').innerHTML = `${starStr} 神接球！接住率 <b>${rate}%</b> —— 你的耳朵和手都好准！`;
      cheerBurst(root, 80);
      try { victoryLightShow(root, { confetti: 90, text: `🏓 接球大师！${rate}%` }); } catch (_) {}
    } else if (stars >= 1) {
      $('#pdl-feedback').innerHTML = `${starStr} 接住率 <b>${rate}%</b>，最佳连击 <b>${game.bestCombo}</b>。盯住快落地的球更容易接，再来一次！`;
      cheerToast(`${starStr} 不错哦！`, root);
    } else {
      $('#pdl-feedback').innerHTML = `这次没接住几个，没关系～先用 🌱 五声 + 🐢 慢速，球上有音名，照着找键就行，很快有手感！`;
    }
    updateHud(); draw(); drawPicker();
  }

  function start() {
    const count = +$('#pdl-count').value;
    const fallMs = +$('#pdl-speed').value;
    game = new PaddleTones({ pool: poolId, count, fallMs, gapMs: Math.round(fallMs * 0.52) });
    voiced.clear(); floats.length = 0; lineFlash = 0;
    t0 = performance.now();
    running = true;
    $('#pdl-start').textContent = '⏹ 停止'; $('#pdl-start').classList.add('running');
    $('#pdl-feedback').innerHTML = byEar ? `🔊 听音接：球落下时会发声，靠耳朵找出是哪个音弹出来接住！` : `开始！音球掉下来，弹出球上写的音把它接住 🏓`;
    paddleOnNote = (note) => handlePlay(note);
    updateHud();
    rafId = requestAnimationFrame(loop);
  }
  ...
  function stop() {
    ...
    running = false; paddleOnNote = null;
    $('#pdl-start').textContent = '▶ 开始'; $('#pdl-start').classList.remove('running');
    if (game && game.settled > 0) finish(); else { $('#pdl-feedback').textContent = '已停止'; draw(); }
  }

  const kb = new PianoKeyboard($('#pdl-kb'), {
    labels: 'c',
    onNoteOn: (m) => { playTone(midiToFreq(m), 0, 0.5); handlePlay(m); },
  });
  kb.scrollToShow(55, 79);

  $('#pdl-start').onclick = () => { if (running) stop(); else start(); };
  $('#pdl-ear').onchange = () => { byEar = $('#pdl-ear').checked; };

  document.addEventListener('ca99:module-change', (e) => {
    if (e.detail !== 'paddle' && running) {
      running = false; paddleOnNote = null; cancelAnimationFrame(rafId);
    }
  });

  drawPicker(); updateHud(); draw();
}
